...
check_file_contains "docker-compose pins jetpack-etcd image tag" "$SCRIPT_DIR/docker-compose.yml" "image: jetpack-etcd"
...
check_file_contains "Script runs all servers+client in one process" "$SCRIPT_DIR/run-etcd-test.sh" "all servers + client in one process"
...
check_file_contains "Script defines 5 host processes" "$SCRIPT_DIR/run-etcd-test.sh" 'host_procs=("h1" "h2" "h3" "h4" "h5")'
check_file_contains "Script validates 5 servers + 5 clients" "$SCRIPT_DIR/run-etcd-test.sh" "5 servers + 5 clients"
...
check_file_contains "Script uses external recovery flow (no failover config)" "$SCRIPT_DIR/run-etcd-test.sh" "WITHOUT failover config"
