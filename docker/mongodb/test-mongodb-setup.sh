...
check_file_contains "docker-compose pins jetpack-mongodb image tag" "$SCRIPT_DIR/docker-compose.yml" "image: jetpack-mongodb"
...
check_file_contains "Script runs all servers+client in one process" "$SCRIPT_DIR/run-mongodb-test.sh" "all servers + client in one process"
...
check_file_contains "Script configures 3 server replicas" "$SCRIPT_DIR/run-mongodb-test.sh" "Config: 1 client, 3 server replicas"
check_file_contains "Script configures 1 client" "$SCRIPT_DIR/run-mongodb-test.sh" "Config: 1 client, 3 server replicas"
...
check_file_contains "Script uses external recovery flow (no failover config)" "$SCRIPT_DIR/run-mongodb-test.sh" "WITHOUT failover config"
...
check_file_contains "Script writes MongoDB primary_elected signal" "$SCRIPT_DIR/run-mongodb-test.sh" "mongo:primary_elected"
echo ""
