...
- **naive_epaxos baseline protocol** — Sibling of naive_raft but with **leader per site**: each client sends its Dispatch to the **local/colocated** naive_epaxos leader (the server on the same zoo host); that leader broadcasts to the 4 remote servers and waits for 2 follower acks (3/5 simple majority counting self) before replying. No log, no election, no dep tracking — a pure distributed-leader + majority-quorum baseline that matches EPaxos's request routing without any of the EPaxos bookkeeping. Implement under `src/deptran/naive_epaxos/`, mirroring the naive_raft wiring ([src/deptran/naive_raft/commo.{h,cc}](src/deptran/naive_raft/), gated branch in [service.cc](src/deptran/service.cc), routing hook in [communicator.cc](src/deptran/communicator.cc)). Key routing difference: instead of `GetLeaderForPartition` returning 1, return the caller's own locale_id so each client hits its colocated server.
- **TLA: base_mongodb.tla + jetpack_mongodb_composition.tla** — Port [mongo-repl-tla](https://github.com/visualzhou/mongo-repl-tla) into [tla/mongodb.tla](tla/), then write [tla/base_mongodb.tla](tla/) as the jetpack-pluggable wrapper (3-D log, hooks for jetpack composition — follow [base_raft.tla](tla/base_raft.tla), [base_copilot.tla](tla/base_copilot.tla), [base_mencius.tla](tla/base_mencius.tla) as references), then [tla/jetpack_mongodb_composition.tla](tla/) that makes the two run together. Matches the existing `jetpack_{raft,copilot,mencius}_composition.tla` pattern.
- **TLA: empty view change for protocols without one** — [base_copilot.tla](tla/base_copilot.tla) and [base_mencius.tla](tla/base_mencius.tla) have no view change of their own, so jetpack composition can't currently trigger view change on top of them. Add a no-op "empty view change" action whose only effect is to signal jetpack's view change. Same treatment needed for [base_mongodb.tla](tla/) above if mongodb also lacks a view change (raft-style — expected).
- **Tune adaptive mode for Jetpack+Raft** — At saturation, `-m 101` should behave like plain Raft (fast-path attempt rate → 0) and hit Raft's peak throughput, not fall below it. Currently jetpack+raft adaptive peaks at 7993 cmd/s vs raft 13984 ([docs/max_throughput_bisection_2026-04-16.md](docs/max_throughput_bisection_2026-04-16.md)). The throttle already monitors ongoing commands / latency / CPU ([src/deptran/rule/coordinator.cc](src/deptran/rule/coordinator.cc)); need to verify the saturation signal actually drives the rate to ~0 rather than oscillating, and that the baseline-raft path runs without residual fast-path CPU overhead once the rate is 0.
