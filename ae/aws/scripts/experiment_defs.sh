...
# ──────────────────────────────────────────────────────────────────────
# AE Group-B baselines (curp / swiftpaxos / epaxos)
# ──────────────────────────────────────────────────────────────────────
#
# These protocols have NO JetPack sibling — they're standalone reference
# baselines. Single config + single -m flag, 1 variant per concurrency
# point. Added per ae/aws/camera-ready/settings.md §3.3 Group B.

LEGACY_BASELINE_LABELS=("curp" "swiftpaxos" "epaxos")
LEGACY_BASELINE_CONFIGS=("none_curp" "none_swiftpaxos" "none_epaxos_corrected")
LEGACY_BASELINE_MODES=("200" "0" "0")        # curp uses MODE_CURP, others use MODE_ORIGINAL
LEGACY_BASELINE_FIXED_CONCS=("concurrent_100" "concurrent_75" "concurrent_100")

CURP_CONCS=(
    concurrent_1 concurrent_10 concurrent_25 concurrent_50 concurrent_75
    concurrent_100 concurrent_150 concurrent_200 concurrent_300 concurrent_500
    concurrent_750
)
SWIFTPAXOS_CONCS=(
    concurrent_1 concurrent_10 concurrent_25 concurrent_50 concurrent_75
    concurrent_100 concurrent_125 concurrent_150 concurrent_175 concurrent_200
    concurrent_250
)
EPAXOS_CONCS=(
    concurrent_1 concurrent_10 concurrent_25 concurrent_50 concurrent_100
    concurrent_150 concurrent_200 concurrent_250 concurrent_300 concurrent_400
    concurrent_500
)
...
concs_array_for() {
    ...
    case "$proto" in
        ...
        *curp)             echo "CURP_CONCS" ;;
        *swiftpaxos)       echo "SWIFTPAXOS_CONCS" ;;
        *epaxos*)          echo "EPAXOS_CONCS" ;;
