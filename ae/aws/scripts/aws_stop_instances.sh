#!/usr/bin/env bash
# Stop all instances listed in scripts/aws_instances.tsv.
# Optional: ONLY_INDICES="00 03" to stop a subset.
set -uo pipefail
...
while IFS=$'\t' read -r idx city region instance_id eip; do
  ...
  state=$(aws ec2 stop-instances --region "$region" --instance-ids "$instance_id" \
