...
def draw_grid(log_dir, out_path, dc=None, dc_label=None):
    fig, axes = plt.subplots(1, 5, figsize=(25, 5.8), sharey=True)
    axes = axes.flatten()
    counts = []
...
def draw_compare(log_dir, out_path, dc=None, dc_label=None):
...
def run_one(result_dir):
    ...
    # Cluster
    grid_path = os.path.join(figs_dir, "latency_cdf_grid.pdf")
    grid_counts = draw_grid(log_dir, grid_path)
    print(f"  [GRID] {grid_path}")
    for title, conc, n in grid_counts:
        print(f"    {title:<10} c={conc:<4}  -> {n} variants drawn")

    cmp_path = os.path.join(figs_dir, "latency_cdf_raft_curp_swiftpaxos_epaxos.pdf")
    cmp_counts = draw_compare(log_dir, cmp_path)
    print(f"  [CMP]  {cmp_path}")
    for label, conc, npts in cmp_counts:
        print(f"    {label:<32} c={conc:<4}  -> {npts} samples")
